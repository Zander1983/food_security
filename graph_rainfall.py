...
milk_rainfall_data['Rainfall'] = pd.to_numeric(milk_rainfall_data['Rainfall'], errors='coerce')
...
# Step 5: Create a plot for 'Rainfall' across months for each year
...
for year, year_data in milk_rainfall_data.groupby('Year'):
    if year in highlight_years:
        ...
        plt.plot(year_data['Month'], year_data['Rainfall'], label=str(year), linewidth=2.5)
    else:
        # Plot all other years in grey
        plt.plot(year_data['Month'], year_data['Rainfall'], color='grey', linewidth=1, alpha=0.6)

# Step 6: Customize the plot
plt.title('Rainfall by Month for Each Year')
plt.xlabel('Month')
